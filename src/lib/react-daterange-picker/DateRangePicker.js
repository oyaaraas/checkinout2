...
const DateRangePicker = React.createClass({
  ...
  getInitialState() {
    const now = new Date();
    const {initialYear, initialMonth, initialFromValue, selectionType, value} = this.props;
  ...
  getDateStates(props) {
    ...
    const shiftDays = this.props.fullDayStates ? 1 : 0;
    ...
    return Immutable.List(actualStates).map(s => {
  ...
  renderCalendar(index) {
    ...
    let {highlightedDate, highlightedRange} = this.state;
  ...
  render() {
    const {paginationArrowComponent: PaginationArrowComponent, numberOfCalendars, stateDefinitions, selectedLabel, showLegend, helpMessage} = this.props;
